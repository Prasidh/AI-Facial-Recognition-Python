...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(len(pixels)):
        ...
        for j in range(len(row)):
            if row[j] != zero:
                non_zero += one
                if j > zero and row[j - one] != row[j]:
                    number_of_breaks += one
                if i <= ((len(pixels) + one) / 2):
                    top_half += one
                if j >= ((len(row) + one) / 2):
                    right_half += one
    """
...
# Initializes all command line arguments
